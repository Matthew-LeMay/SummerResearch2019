...
def find_distances(task_name, tap_feature_name, reading_value, list_of_users = None):
    ...
    for u1 in list_of_users:
        for session1 in get_user_session_ids_for_task(u1, task_name):
            path = 'Plots/Research/'+u1+'/'+task_name+'/'+session1+'/'+tap_feature_name+'/'
            file_name = path + 'Distances.csv'

            # Make sure we don't overwrite existing data
            listOfFiles = os.listdir(path)
            if file_name in listOfFiles:
                continue
...
def classification_is_correct(user1, session1, user2, session2, task_name, threshold_dict):
    """
    Returns True if the sessions are different and would be classified as such, and False otherwise
    """
    tap_feature_names = [tap_file_to_feature_name[t] for t in tap_file_names]
    reading_values = ['accelX', 'accelY', 'accelZ', 'gyroX', 'gyroY', 'gyroZ']
    total_distance_from_thresholds = 0
    for t in tap_feature_names:
        for r in reading_values:
            dist = distance(user1, session1, user2, session2, task_name, t, r)
            th = threshold_dict[(t, r)]
            if th == None:
                continue
            [lower_threshold, mid_threshold, upper_threshold] = th
            if dist < lower_threshold and dist <= upper_threshold:
                return (user1 == user2)
            elif dist > upper_threshold and dist >= lower_threshold:
                return (user1 != user2)
            else:
                total_distance_from_thresholds += (dist - mid_threshold)
    if total_distance_from_thresholds < 0:
        return (user1 == user2)
    else:
        return (user1 != user2)

def threshold_dictionary(task_name, session_dictionary):
    print("Building a threshold dictionary...")
    tap_feature_names = [tap_file_to_feature_name[t] for t in tap_file_names]
    reading_values = ['accelX', 'accelY', 'accelZ', 'gyroX', 'gyroY', 'gyroZ']
    dic = {}
    for t in tap_feature_names:
        ...
        for r in reading_values:
            print(r)
            try:
                [[mean_same, sd_same, max_same], [mean_different, sd_different, min_different]] = \
                    mean_distance_same_different(task_name, t, r, session_dictionary)
                mid_threshold = (mean_different * sd_same + mean_same * sd_different) / (sd_same + sd_different)
                lower_threshold = min_different
                upper_threshold = max_same
                dic[(t, r)] = [lower_threshold, mid_threshold, upper_threshold]
            except KeyError:
                # If something goes wrong, we'll ignore that for now
                print("Unusable combination of tap feature and reading!")
                dic[(t, r)] = None
    print("Threshold dictionary complete!")
    return dic

def mean_distance_same_different(task_name, tap_feature_name, reading_value, session_dictionary):
    """
    Actually returns: [[mean_same, sd_same, max_same], [mean_different, sd_different, min_different]]
    """
    list_of_users = session_dictionary.keys()
    tap_file_name = tap_feature_to_file_name[tap_feature_name]
    same_user = []
    different_user = []
    for u1 in list_of_users:
        for session1 in session_dictionary[u1]:
            path = 'Plots/Research/'+u1+'/'+task_name+'/'+session1+'/'+tap_feature_name+'/'
            file_name = path + 'Distances.csv'
            ...
            with open(file_name, mode = 'w') as file:
                ...
                #     continue
                for u2 in list_of_users:
                    for session2 in session_dictionary[u2]:
                        if session1 >= session2:
                            continue # Only investigate each pair once
                        #print(session2)
                        # if (not above_threshold(u2, session2, task_name, tap_file_name)):
                        ...
                        writer.writerow([session2, d])
            file.close()
    return [[mean(same_user), sd(same_user), max(same_user)], \
            [mean(different_user), sd(different_user), min(different_user)]]

def test_identification(task_name, list_of_users):
    training_sessions = {}
    testing_sessions = {}
    for user in list_of_users:
        sessions = get_user_session_ids_for_task(user, task_name)
    ...
    thresh_dict = threshold_dictionary(task_name, training_sessions)
    ...
    for user1 in list_of_users:  
        for session1 in testing_sessions[user1]:
            for user2 in list_of_users:
                for session2 in testing_sessions[user2]:
                    if session1 >= session2:
                        continue # Only compare each pair once
                    if classification_is_correct(user1, session1, user2, session2, task_name, thresh_dict):
                        num_successes += 1
                    else:
                        misclassifications.append([session1, session2])
                    num_attempts += 1
    ...
    if (len(misclassifications) > 0):
        print("Unsuccessful classifications:")
        for mis in misclassifications:
            print(mis[0],mis[1],"were misclassified.")
